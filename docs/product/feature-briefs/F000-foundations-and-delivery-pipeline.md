...
- **Monorepo & app skeleton:** pnpm/turbo workspace (`apps/web`, `packages/shared`, `packages/ui`); `.node-version` + `packageManager` pins; React Router framework mode (major decided here against the current official Cloudflare template — recorded in tech-context §1) + `@cloudflare/vite-plugin` + Tailwind v4; Hono mounted for `/api`+`/ws`; strict tsconfig; Biome; Vitest + workers-pool smoke test; security-header baseline middleware (CSP report-only).
...
- **Environments:** `wrangler.jsonc` per tech-context §3 (names/ids only, never secrets); `.dev.vars.example`; committed `docs/runbooks/resource-bootstrap.md` (exact D1/KV creation commands per env).
