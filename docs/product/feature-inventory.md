...
> Provenance: P1 interview 2026-07-06 · **P2 final product interview 2026-07-12** · **P3 milestone restructuring and release versioning 2026-07-12** · **FR final pre-tech-interview review 2026-07-13** (decision-log §FR: public-beta risk acceptance, concurrent-presence capacity model, 14-day create-ahead default, precision corrections) · **AF additional-features review 2026-07-15** (decision-log §AF: dual-mode admin wall, health endpoint FND-07, self-hosting OPS-07, voucher email lock, ops-counter model, SEO + default-deny noindex, hardening addenda). Status values: **confirmed** (explicit interview decision) · **assumed** (Claude's call under interview rules, veto anytime) · **open** (deferred, owner noted) · **parked** (deliberately shelved) · **rejected** (decided against; kept for context). Current delivery order: **M1 → M2 → M3 → M4 → M5 → M6 → M7 → M8 → M9 → M10 → M11 → M12 → M13-if-ever**. Stable feature IDs and all P1/P2 scope are preserved; P3 changes sequencing, milestone ownership, and target versions only.
...
| FND-03 | Security & hardening baseline | Security headers (incl. `Referrer-Policy: no-referrer` and default-deny search-engine indexing — `X-Robots-Tag: noindex` everywhere, both envs; F007 allowlists marketing surfaces on PRD — AF-1/AF-6), request-body size caps, WS origin allowlist, strict TS/zod boundaries, structured logs, keyless `ops_counters` store + stats service seam (AF-3), audit-log table (privileged admin/entitlement actions only — never moderator in-event actions), soft-delete semantics. (The former `users.role` column was dropped by AF-7 — the admin is not a product user.) | M1 | `v0.1.0` | confirmed |
...
| FND-07 | Health endpoint | `GET /health` behind a deploy-time enable/disable var (a Wrangler var, not KV config — must work when KV is the failure); 200 only if D1 + KV respond, else 503; minimal body, never behind auth, per-IP burst ceiling; reports infrastructure health (deliberately truthful-200 during an M7 emergency stop). (F008; AF-8) | M1 | `v0.1.0` | confirmed |
...
| id | name | description | milestone | target version | status |
|---|---|---|---|---:|---|
| AUTH-01 | Magic-link auth | Email-only moderator auth (E-1): hashed single-use token (15 min), ~30-day session cookie, anti-enumeration incl. the per-email request cap (pulled forward from SAFE-02 — tech review 2026-07-14); banned addresses silently stop receiving links (F002). | M2 | `v0.2.0` | confirmed |
...
| SAFE-02 | Rate limiting | Per-action limits keyed ip/uid/sid (Workers binding or KV fallback); rescheduling is a rate-limited action. The magic-link per-email cap ships earlier, with AUTH-01 / M2 (tech review 2026-07-14). | M4 | `v0.4.0` | confirmed |
| MKT-01 | Landing page | Moderator-pitch hero (line pillar), 3-step how-it-works, create CTA, prominent join-code box, honest privacy block, visual line mock; clean — depth in sub-pages; no pricing, no repo link, no SLA disclaimers on the hero. SEO per the AF-5 checklist (per-locale titles/descriptions, canonical, hreflang, OG/Twitter, JSON-LD, semantic headings). | M4 | `v0.4.0` | confirmed |
| MKT-02 | Sub-pages | How it works, FAQ (incl. honest outage entry), privacy (truthful lifetimes/deletion/export responsibility), terms (best-effort/no-SLA phrased as what MicLine does), Impressum, contact (F007). F007 also flips the marketing surfaces out of the default-deny noindex baseline (PRD only) and ships robots.txt + sitemap.xml covering marketing pages only (AF-5/AF-6). | M4 | `v0.4.0` | confirmed |
...
| id | name | description | milestone | target version | status |
|---|---|---|---|---:|---|
| STD-05 | Recap & export | Rank 1 (raised from 5; closes OQ5): post-end recap + one-click export in CSV, JSON, and Line snapshot (HTML — self-contained one-pager mirroring the queue incl. pool; naming WIP). Contents: everything (questions, attribution, times, final statuses incl. declined + reasons, merge structure, votes, follow-up flags, primer, stats). No export-time filters. Responsibility line at export + privacy page. Entitlement-independent, never gated. Exports render user content inert: CSV formula-escaping, fully-escaped HTML snapshot with no executable script (AF-1). Format list re-validated at M5 kickoff. | M5 | `v0.5.0` | confirmed |
...
| id | name | description | milestone | target version | status |
|---|---|---|---|---:|---|
| ADM-01 | Admin bootstrap | Admin identity exclusively platform-side (AF-7): the ADM-02 wall is the admin auth — ADMIN_EMAILS is the Access-mode allowlist, password mode needs no email; magic-link auth plays no role on /admin; no in-product admin management, no product-user role; recovery = update secret / Cloudflare dashboard. | M6 | `v0.6.0` | confirmed |
| ADM-02 | Admin auth wall | /admin behind one dedicated mandatory auth layer (not magic-link + stacked factor). Mechanism (Cloudflare Access vs TOTP vs passkey) decided at the tech interview for the M6 specification with pricing/limits/exit-cost criteria; hard requirement: emergency reset always possible via Cloudflare account access alone — no lockout dead-end. Step-up confirmation on destructive actions regardless. | M6 | `v0.6.0` | confirmed (mechanism amended 2026-07-15, AF-7: dual-mode — ADMIN_PASSWORD quick-start with signed 5-day session, or Cloudflare Access + one-time PIN, recommended and always winning; the wall is the admin auth; fails closed unconfigured — tech-context §11.4) |
...
| ADM-05 | Metrics wall | Own-counter stats: events (created/active/week), participants, questions, emails sent, Turnstile failures, rate-limit hits, storage growth; € projection arrives with the M12 / `v1.0.0` cost model. Full counter catalog + keyless `ops_counters` store: tech-context §16 (AF-3) — counters accumulate from each signal's introducing milestone (M2 auth/email, M3 events/presence, M4 abuse/purge/feedback, M8 vouchers), so this wall opens with history. | M6 | `v0.6.0` | confirmed |
...
| ADM-07 | Platform controls | Service notice banner (placement: always landing/auth/console; participant surfaces + board only when flagged as affecting live events), creation kill-switch (UI over the M1 / `v0.1.0` flags); invite-only mode toggle activates with M8 / `v0.8.0`. Config/flag writes in the admin UI surface a propagation notice — changes can take up to ~90 s (KV; tech review 2026-07-14). | M6 | `v0.6.0` | confirmed |
...
| ENT-03 | Invite codes (vouchers) | Bundles {may_register? + grants + limit raises + validity} redeemed into account wallet; multi-use via max_redemptions; two clocks (redeem-by, grant validity); god-voucher preset; optional admin-set **email lock** (AF-2): locked codes are single-use and redeemable only by that normalized address — handed-out codes can't be transferred; the locked email is purged with the expired code. Design details (format, QR, custom message, generation UX, clock defaults, email-lock UX) at M8 kickoff. | M8 | `v0.8.0` | confirmed |
...
| OPS-07 | Self-hosting deployment paths | Deploy-to-Cloudflare button + CLI scaffold via `npm create cloudflare -- --template` (C3 — no own npm package) + SELF_HOSTING guide; honest one-click semantics: a deployed shell plus printed setup steps (email-domain onboarding, secrets, admin wall — which fails closed until configured, AF-7); forward-compat demands on M1–M4 recorded in AF-9 (no hard-coded hostnames, deploy-shaped config path, /health); button/C3 contracts verified at implementation. | M12 | `v1.0.0` | confirmed |
