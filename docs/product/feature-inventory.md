...
| ADM-02 | Admin auth wall | /admin behind one dedicated mandatory auth layer (not magic-link + stacked factor). Mechanism (Cloudflare Access vs TOTP vs passkey) decided at the tech interview for the M6 specification with pricing/limits/exit-cost criteria; hard requirement: emergency reset always possible via Cloudflare account access alone — no lockout dead-end. Step-up confirmation on destructive actions regardless. | M6 | `v0.6.0` | confirmed (mechanism: Cloudflare Access + one-time PIN — tech interview 2026-07-14, tech-context §11.4) |
