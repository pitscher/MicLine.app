...
```
Phase 0            Phase 1–4 (planning)              per feature ×8            ship
┌─────────┐   ┌──────────────────────────────┐   ┌──────────────────┐   ┌───────────┐
│ 02 setup│ ▶ │ 04 brainstorm → tech context │ ▶ │ 05 loop on each  │ ▶ │ Release → │
│ 03 read │   │   → constitution → milestones│   │ 06 F000…F007     │   │ PRD (03§F)│
└─────────┘   └──────────────────────────────┘   └──────────────────┘   └───────────┘
                     (01 decides which model does what, throughout)
     then: 07 repeats the pattern for M2–M5 · 08/09 are references you'll revisit · 10 is where you look up after each milestone
```
...
| 4 | [04-product-definition.md](04-product-definition.md) | Prompts P1–P4: brainstorm interview → tech context → constitution → milestones & epic briefs | Days 1–3 (Fable window) |
...
| 6 | [06-m1-plan.md](06-m1-plan.md) | F000–F007 in order, each with scope + plan addendum + special notes; the optional Lunaria loop; the M1 release gate | Throughout M1 |
| 7 | [07-m2-to-m5.md](07-m2-to-m5.md) | Kickoff ritual + playbooks: admin & vouchers/entitlements, parked monetization, UI overhaul, longevity | At each milestone start |
...
| 10 | [10-whats-next.md](10-whats-next.md) | Looking up after shipping: product horizons, infra hardening, process evolution, the open-to-contributors checklist | After the M1 gate; again after every milestone |
...
| — | [inputs/tech-decisions.md](inputs/tech-decisions.md) | The distilled, pre-verified stack decisions (input to prompt P2; provenance-only after Gate 2) — lands at `docs/inputs/` | Fed to Claude, not you |
...
**Milestones: numbering ≠ execution order, and nothing is final until Gate 4.** M1–M5 appear throughout this guide by name so ideas don't get lost, but they are **working titles**: prompt P4 ([file 04 §6](04-product-definition.md)) is where you and Claude challenge, rename, merge, or re-cut them — after Gate 4, `docs/product/milestone-map.md` is the canonical set and every M-reference in this guide maps onto it. Execution order is stated once, plainly, in [file 07](07-m2-to-m5.md): **M1 → M2 → M4 → M5 → M3-if-ever** (monetization keeps its historical number but runs last by design).
...
**Lunaria** — an open-source, git-history-based translation-status dashboard; an optional tooling loop in [file 06](06-m1-plan.md).
