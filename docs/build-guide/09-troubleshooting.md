...
3. Entitlement seam, audit log, soft-delete, structured logs, ops counters, and centralized design tokens are pulled into M1 as forward-compat demands even though their consuming UIs land in M6/M8/M11/M12. *(The former `users.role` demand was dropped 2026-07-15 — AF-7: the admin wall is the admin auth; the admin is not a product user.)*
